...
{   // 操作符，参数1在符号表的索引，参数2在符号表的索引，结果在符号表中的索引
    ...
    try {
        if (op == "neg") { // 取负
            ...
            // if()
            symbolTable[resultIndex].symValue;
        }
        else if (op == "<") {
            // if()
            symbolTable[resultIndex].symValue;
    ...
    if (op == ">" || op == "<" || op == ">=" || op == "<=" || op == "==") {
        symItem M;
        M.symName = "M"+to_string(quaSequence.size()+3);
        M.position = quaSequence.size() + 3;
...
// A-> while ( C );
void GrammaticalAnalysis::AWhileCB() {
    symItem B, C, WHILE;
    for (int popTime = 0; popTime < 7; popTime++)
    { // E->E + E 要出栈三次，进栈一次
        if (popTime == 1)
            B = opStack.top();
        if (popTime == 4)
    ...
    opStack.push(symItem{ grammar.productionTable[1].left }); // 把"A"push进符号栈，但还没和语义栈关联，下面进行

    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);


    WHILE.symValue = to_string(Mstack.top().result.position - 2);
    WHILE.position = quaSequence.size() + 2;
    Mstack.pop();
    semanticStack.pop();
    // 输出四元式
    int arg1Index = C.position;
    //cout << quaSequence.size() << " (" << "j_true" << ",";
    //arg1Index != -1 ? cout << symbolTable[arg1Index].symName : cout << " ";
    //cout << ",";
    //cout << " ";
    //cout << "," << Do.symValue << ")" << endl;
    // 插入到四元式序列中
    quaSequence.push_back(
        quaternion{ "while{}",
        arg1Index == -1 ? symItem{ " " } : symbolTable[arg1Index],
        symItem{ " " },
        symItem{ WHILE.symValue,to_string(WHILE.position) } });
}            
// C->E ROP E
void GrammaticalAnalysis::C_E_R_E() {
    symItem E1, ROP, E2;
    ...
    opStack.push(symItem{ grammar.productionTable[2].left }); // 把"C"push进符号栈，但还没和语义栈关联，下面进行
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);

    symItem T = newtemp();
    GEN(ROP.symValue, E1.position, E2.position, T.position);
    tempVarNum--;
...
// B->S;B
void GrammaticalAnalysis::B_S_B() {
    symItem E1 = opStack.top();
    int popNum = grammar.productionTable[3].right.size();
    while (popNum)
    {
        statusStack.pop();
        opStack.pop();
    ...
    // opStack.top().symValue = to_string(0);         // 四元式地址
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
}               
// B->S;
void GrammaticalAnalysis::B_S() {
    symItem E1 = opStack.top();
    int popNum = grammar.productionTable[4].right.size();
    while (popNum)
    {
        statusStack.pop();
        opStack.pop();
    ...
    // 把"E"push进符号栈，但还没和语义栈关联，下面进行
    opStack.push(symItem{ grammar.productionTable[4].left });
    //  opStack.top().symValue = to_string(0);         // 四元式地址
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
}   
// B->A
void GrammaticalAnalysis::B_A() {
    symItem E1 = opStack.top();
    int popNum = grammar.productionTable[5].right.size();
    while (popNum)
    {
        statusStack.pop();
        opStack.pop();
    ...
    // 把"E"push进符号栈，但还没和语义栈关联，下面进行
    opStack.push(symItem{ grammar.productionTable[5].left });
    //  opStack.top().symValue = to_string(0);         // 四元式地址
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
}                   
...
void GrammaticalAnalysis::E_minE() {
    symItem E1 = opStack.top();
    int popNum = grammar.productionTable[7].right.size();
    while (popNum)
    {
        statusStack.pop();
        opStack.pop();
...
void GrammaticalAnalysis::E_F() {
    ...
    opStack.push(symItem{ grammar.productionTable[8].left });
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
...
void GrammaticalAnalysis::E_EaddF() {
    ...
    opStack.push(symItem{ grammar.productionTable[9].left }); // 把"E"push进符号栈，但还没和语义栈关联，下面进行
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
...
void GrammaticalAnalysis::E_EsubF() {
    ...
    opStack.push(symItem{ grammar.productionTable[10].left }); // 把"E"push进符号栈，但还没和语义栈关联，下面进行
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
    // 同步执行相应的语义动作
    symItem T = newtemp();
...
void GrammaticalAnalysis::F_FmulG() {
    ...
    opStack.push(symItem{ grammar.productionTable[12].left }); // 把"E"push进符号栈，但还没和语义栈关联，下面进行
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
...
void GrammaticalAnalysis::F_FdivG() {
    ...
    opStack.push(symItem{ grammar.productionTable[13].left }); // 把"E"push进符号栈，但还没和语义栈关联，下面进行
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
...
void GrammaticalAnalysis::G_i() {
    int popNum = grammar.productionTable[14].right.size();
    while (popNum) {
        statusStack.pop();
        opStack.pop();
        popNum--;
    ...
    opStack.push(symItem{ grammar.productionTable[14].left });
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
    // G->i规约,将E与语义栈栈顶值关联之后
    opStack.top().position = entryMap[semanticStack.top()];
...
void GrammaticalAnalysis::G_n() {
    int popNum = grammar.productionTable[15].right.size();
    while (popNum) {
        statusStack.pop();
        opStack.pop();
        popNum--;
    ...
    // 把"G"push进符号栈，但还没和语义栈关联，下面进行
    opStack.push(symItem{ grammar.productionTable[15].left });
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
    // G->n规约,将E与语义栈栈顶值关联之后
    opStack.top().position = entryMap[semanticStack.top()];
...
void GrammaticalAnalysis::G_E() {
    ...
    // 把"G"push进符号栈，但还没和语义栈关联，下面进行
    opStack.push(symItem{ grammar.productionTable[16].left });
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
...
// ROP-> >
void GrammaticalAnalysis::R_greater() {
    // 出栈
    symItem ropVal = opStack.top();
    statusStack.pop();
    opStack.pop();
    // 入栈 + 语义动作
    opStack.push(symItem{ grammar.productionTable[17].left });
    opStack.top().symValue = ropVal.symName;
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
}     
// ROP-> <
void GrammaticalAnalysis::R_less() {
    // 出栈
    symItem ropVal = opStack.top();
    statusStack.pop();
    opStack.pop();
    // 入栈 + 语义动作
    opStack.push(symItem{ grammar.productionTable[18].left });
    opStack.top().symValue = ropVal.symName;
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
}     
// ROP-> >=
void GrammaticalAnalysis::R_ge(){
    // 出栈
    symItem ropVal = opStack.top();
    statusStack.pop();
    opStack.pop();
    // 入栈 + 语义动作
    opStack.push(symItem{ grammar.productionTable[19].left });
    opStack.top().symValue = ropVal.symName;
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
}
// ROP-> <=
void GrammaticalAnalysis::R_le(){
    // 出栈
    symItem ropVal = opStack.top();
    statusStack.pop();
    opStack.pop();
    // 入栈 + 语义动作
    opStack.push(symItem{ grammar.productionTable[20].left });
    opStack.top().symValue = ropVal.symName;
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
}         
// ROP-> ==
void GrammaticalAnalysis::R_eq(){
    // 出栈
    symItem ropVal = opStack.top();
    statusStack.pop();
    opStack.pop();
    // 入栈 + 语义动作
    opStack.push(symItem{ grammar.productionTable[21].left });
    opStack.top().symValue = ropVal.symName;
    int curStatus = statusStack.top();
    statusStack.push(gotoTable[curStatus][grammar.getVIndex(opStack.top().symName)]);
}                    
...
void GrammaticalAnalysis::grammaticalTranslation()
{
    ...
    while (!opStack.empty() && !errFlag) {
        ...
        else if (curWord.sym == 36) {
            semanticStack.push(curWord.token+ to_string(doIndex));    // 语义栈
            // 将符号加入符号表，并添加入口地址映射
            symItem tempSym;
            tempSym.symName = curWord.token + to_string(doIndex);
            tempSym.symValue = to_string(quaSequence.size());     // do 记录当前四元式地址位置
            tempSym.position = symbolTable.size();
            symbolTable.push_back(tempSym);
            entryMap[tempSym.symName] = tempSym.position;
            doIndex++;
